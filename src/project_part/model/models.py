...
# from .finance import PagamentoQuota, Doacao
...
class DespesaStatusEnum(str, Enum):
    PENDING = 'PENDING'
    APPROVED = 'APPROVED'
    REJECTED = 'REJECTED'
    CANCELLED = 'CANCELLED'

class FinalidadeFundoEnum(str, Enum):
    EVENTOS = 'EVENTOS'
    FORMACOES = 'FORMACOES'
    COMICIOS = 'COMICIOS'
    MATERIAL = 'MATERIAL'
...
class Doacao(Base):
    ...
    doador: Mapped[Optional['User']] = relationship(
        'User', foreign_keys=[user_id], back_populates='doacoes', lazy="joined"
    )
...
class SolicitacaoFundo(Base):
    """Pedido de fundos da província (não ligado a um militante)."""
    __tablename__ = 'solicitacoes_fundo'

    id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), primary_key=True, default=uuid.uuid4
    )

    # Território que pede (obrigatório)
    provincia_id: Mapped[int] = mapped_column(
        Integer,
    ...
    # Opcional: se no futuro município também pedir
    ...
    finalidade: Mapped[FinalidadeFundoEnum] = mapped_column(nullable=False)
    descricao: Mapped[str] = mapped_column(String(500), nullable=False)
    quantia: Mapped[Decimal] = mapped_column(Numeric(15, 2), nullable=False)
    moeda: Mapped[str] = mapped_column(String(3), default='AOA', nullable=False)

    status: Mapped[DespesaStatusEnum] = mapped_column(
        default=DespesaStatusEnum.PENDING,
        nullable=False,
        index=True,
    )
    observacao: Mapped[str | None] = mapped_column(TEXT, nullable=True)  # motivo rejeição / nota
